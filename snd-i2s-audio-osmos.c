 ...
 * =====================================================================================
 *
 *       Filename:  snd-i2s-audio-osmos
 *
 *    Description:  I2S audio kernel module
 *
 *        Version:  0.1.0
 *        Created:  2022-06-4
 *       Revision:  none
...
static struct asoc_simple_card_info default_card_info = {
  .card = "snd_osmos_i2s_card",       // -> snd_soc_card.name
...
int i2s_audio_init(void)
{
  ...
  printk(KERN_INFO "snd-i2s-audio-osmos: Version %s\n", SND_I2S_AUDIO_OSMOS_VERSION);
  ...
  printk(KERN_INFO "snd-i2s-audio-osmos: Setting platform to %s\n", card_platform);
 ...
 */
void i2s_audio_exit(void)
{
  platform_device_unregister(&card_device);
  pr_alert("i2s audio module unloaded\n");
}

// Plumb it up
module_init(i2s_audio_init);
module_exit(i2s_audio_exit);
MODULE_DESCRIPTION("Audio for for i2s based mic & amp for osmos & asper");
MODULE_AUTHOR("Ishant Pundir");
